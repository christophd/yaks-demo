...
from('platform-http:/fruits')
    ...
    .setHeader("CE-Type", constant("org.citrusframework.yaks.fruits"))
    .choice()
        .when().simple('${body[nutrition][sugar]} <= 5')
            .setHeader("CE-Subject", constant("low-sugar"))
        .when().simple('${body[nutrition][sugar]} > 5 && ${body[nutrition][sugar]} <= 10')
            .setHeader("CE-Subject", constant("medium-sugar"))
        .otherwise()
            .setHeader("CE-Subject", constant("high-sugar"))
    .end()
    .marshal().json()
    .log('sending ${body}')
    .to('knative:channel/fruits')
